...
export function EventPartsEditor({ stages, onChange, defaults, section, services = [] }: {
  ...
  return <section className="mt-5 space-y-3">
    ...
    {stages.map((part, index) => <div key={part.id} className="rounded-2xl border border-neutral-200 bg-white p-4">
      ...
      {section === "structure" && <div className="mt-3 grid gap-3 sm:grid-cols-2">
        ...
        <fieldset className="sm:col-span-2"><legend className="text-sm">Services for {part.label}</legend><p className="mb-2 text-xs text-neutral-500">Unassigned services remain with the overall event.</p><div className="flex flex-wrap gap-3">{services.map((service) => <label key={service} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={part.services?.includes(service) ?? false} onChange={(event) => update(part.id, { services: event.target.checked ? [...(part.services ?? []), service] : part.services?.filter((item) => item !== service) })} />{service}</label>)}</div></fieldset>
      </div>}
