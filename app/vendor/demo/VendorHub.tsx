...
const sections = ["Calendar", "Overview", "Inquiries", "Events", "Availability", "Services", "Business Profile", "Setup", "Account"] as const;
...
export function VendorHub() {
  ...
  if (!state) return <main className="min-h-screen ui-soft p-8"><Logo /><p className="mt-10" role="status">Opening your Vendor Demo…</p></main>;
  ...
  function eventList(events: VendorEvent[]) { return events.length ? <div className="divide-y divide-neutral-100">{events.map(event => <button key={event.id} onClick={() => openEvent(event)} className="flex w-full flex-wrap items-center justify-between gap-3 py-4 text-left hover:opacity-80 focus-visible:outline-2 focus-visible:outline-[#8A6A16]"><div><p className="font-semibold">{event.name}</p><p className="mt-1 text-sm ui-muted">{dateLabel(event.date)} · {event.start}–{event.end}</p><p className="mt-1 text-xs ui-muted">{event.client} · {sourceLabel(event)}</p></div><Badge event={event} /></button>)}</div> : <p className="py-4 text-sm ui-muted">No events here yet. Add an external booking to start organizing your schedule.</p>; }
  function dayPanel() { return <Panel title={dateLabel(selectedDate)}><div className="flex flex-wrap items-center gap-3"><span className="rounded-full bg-neutral-100 px-3 py-1 text-sm">{schedule.label}</span><span className="text-sm ui-muted">{schedule.hours.enabled ? `${schedule.hours.start}–${schedule.hours.end} working hours` : "No normal working hours"}</span></div>{eventList(schedule.events)}<div className="mt-4 flex flex-wrap gap-2"><button className={buttonClass} onClick={() => addEvent(selectedDate)}>Add event on this date</button><button className={secondaryClass} onClick={() => save({ ...state!, blocked: schedule.blocked ? state!.blocked.filter(d => d !== selectedDate) : [...state!.blocked, selectedDate] }, schedule.blocked ? "Date unblocked. Existing events still reserve their time." : "Date blocked. Existing events remain visible.")}>{schedule.blocked ? "Unblock date" : "Block date"}</button></div><p className="mt-4 text-xs leading-5 ui-muted">Confirmed events reserve their time. Tentative events and holds flag possible conflicts. Blocking a date closes the entire day without deleting events.</p></Panel>; }
  ...
  return <div className="vendor-hub min-h-screen bg-[#F7F6F2] ui-text lg:grid lg:grid-cols-[240px_minmax(0,1fr)]">
    ...
    <main className="min-w-0 px-4 py-7 sm:px-8 lg:px-10 lg:py-10">
      ...
      {editingEvent ? <EventForm key={editingEvent.id} event={editingEvent} state={state} onCancel={() => setEditingEvent(null)} onSave={event => { save({ ...state, events: [...state.events.filter(e => e.id !== event.id), event] }, "Event saved. Calendar and availability updated."); setEditingEvent(null); setEventId(event.id); setSelectedDate(event.date); }} /> : current ? <div className="space-y-5">
        ...
        <Panel title="Event workspace"><div className="flex flex-wrap items-center gap-3"><Badge event={current} /><span className="text-sm ui-muted">{sourceLabel(current)}</span></div><dl className="mt-6 grid gap-6 sm:grid-cols-2">{[["Client", current.client], ["Date and time", `${dateLabel(current.date)} · ${current.start}–${current.end}`], ["Location", current.location], ["Service", current.service], ["Event type", current.type]].map(([label,value]) => <div key={label}><dt className="text-xs uppercase tracking-wider ui-muted">{label}</dt><dd className="mt-2 font-medium">{value}</dd></div>)}</dl><p className="mt-6 whitespace-pre-wrap text-sm leading-6 ui-muted">{current.notes || "No notes added yet."}</p>{scheduleWarnings(state, current).length > 0 && <p className="mt-5 rounded-xl bg-amber-50 p-3 text-sm text-amber-900">{scheduleWarnings(state, current).join(" ")}</p>}<button className={`${secondaryClass} mt-6`} onClick={() => setEditingEvent(current)}>Edit event</button></Panel>
        ...
        <Panel title="Quotes and payments">{current.requestId && <Link className={`${secondaryClass} mb-4 inline-block`} href={`/requests/${current.requestId}`}>View connected customer request</Link>}<p className="text-sm ui-muted">Not connected in this demo. Saving an event does not send a quote, collect a deposit or confirm a customer booking.</p></Panel>
      </div> : <>
        {section === "Overview" && <div className="space-y-6"><div className="grid gap-4 sm:grid-cols-3">{[["Today", `${state.events.filter(e => e.date === today).length} events`, "Calendar"], ["Upcoming", `${upcoming.length} events`, "Events"], ["Profile setup", `${setup.filter(s => s.done).length} of ${setup.length} complete`, "Setup"]].map(([label,value,target]) => <button key={label} onClick={() => navigate(target as Section)} className="rounded-2xl border ui-border ui-surface p-6 text-left"><p className="text-sm ui-muted">{label}</p><p className="mt-3 text-2xl font-semibold">{value}</p><p className="mt-3 text-xs text-[#8A6A16]">Open {target.toLowerCase()} →</p></button>)}</div><div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]"><Panel title="Upcoming events">{eventList(upcoming.slice(0,4))}</Panel><div className="space-y-6"><Panel title="Next actions"><div className="flex flex-col items-start gap-3"><button className={secondaryClass} onClick={() => addEvent()}>Add external booking</button><button className={secondaryClass} onClick={() => navigate("Availability")}>Manage availability</button><button className={secondaryClass} onClick={() => navigate("Setup")}>Complete business setup</button></div></Panel><Panel title="Arivvio opportunities"><p className="text-sm leading-6 ui-muted">Review browser-local customer inquiries, record demo quotes and connect them to tentative calendar events.</p><button className={`${secondaryClass} mt-4`} onClick={() => navigate("Inquiries")}>Review inquiries</button></Panel></div></div></div>}
        {section === "Inquiries" && <Inquiries state={state} save={save} openEvent={openEvent} />}
