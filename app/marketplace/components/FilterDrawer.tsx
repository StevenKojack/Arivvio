...
export function MarketplaceFilters(p: FilterProps) {
  ...
  return <div className="ui-surface rounded-2xl border ui-border p-4">
    <div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-semibold">Filters</h2><button className="text-xs underline" onClick={p.onClear}>Clear all</button></div>
    {!!active.length && <div aria-label="Active filters" className="mb-3 space-y-1">{active.map((filter, index) => <button key={`${filter.label}-${index}`} className="flex w-full items-center justify-between gap-2 rounded-lg ui-soft px-2 py-1 text-left text-xs" onClick={filter.remove} aria-label={`Remove filter ${filter.label}`}>{filter.label}<span aria-hidden>×</span></button>)}</div>}
