"use client";
import { reasonAboutBudget } from "@/lib/event-intelligence/budget";
...
export function EventHubSummary({ request }: { request: DemoQuoteRequest }) {
  ...
  const budget = reasonAboutBudget({ overall: event.budget, stages: event.profile?.stages, estimates: request.opportunities.map(item => item.estimate), quotes: quotes.map(item => item.response?.price ?? 0) });
  const estimates = budget.estimates;
  const quoted = budget.quotes;
  return <section className="my-6 hub-card p-5 sm:p-7" aria-label="Event hub">
    ...
    {section === "Budget" && <><h3 className="font-semibold">Budget snapshot</h3><dl className="mt-4 grid gap-4 sm:grid-cols-3">{[["Overall budget", formatMoney(event.budget)], ["Requested estimates", formatMoney(estimates)], ["Demo quotes received", `${formatMoney(quoted)} · ${quotes.length} responses`]].map(([label,value]) => <div key={label}><dt className="text-xs ui-muted">{label}</dt><dd className="mt-2 font-semibold">{value}</dd></div>)}</dl>{event.budget > 0 && estimates > event.budget && <p className="mt-4 text-sm">Requested estimates exceed your overall budget by {formatMoney(estimates - event.budget)}.</p>}<p className="mt-4 text-sm ui-muted">{budget.unallocated !== null && event.profile?.stages.some(stage => stage.budget !== undefined) ? `${formatMoney(budget.unallocated)} unallocated across parts. ` : ""}{budget.remainingAgainstEstimates !== null ? `${formatMoney(budget.remainingAgainstEstimates)} remaining against requested estimates.` : "Set an overall budget to compare estimates."}</p>{budget.signals.filter(signal => signal.startsWith("Stage")).map(signal => <p key={signal} className="mt-2 text-sm">{signal}</p>)}<p className="mt-4 text-xs ui-muted">Estimates and quotes are separate comparison totals, not amounts paid or committed. Stage allocations are part of the overall budget.</p></>}
